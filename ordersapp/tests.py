...
class TestOrdersAppModelsMethods(TestCase):
    def setUp(self) -> None:
        ...
        self.order_1.orderitems.create(order=self.order_1,
        ...
        self.order_item_1 = self.order_2.orderitems.create(order=self.order_2,
                                                           product=self.product_1,
                                                           quantity=20)
        self.order_item_2 = self.order_2.orderitems.create(order=self.order_2,
                                                           product=self.product_2,
                                                           quantity=70)
    ...
    def test_order_get_summary(self):
        self.assertEqual(self.order_1.get_summary(), {'total_cost': 16000, 'total_quantity': 9})
    ...
    def test_order_delete(self):
        ...
        for n, item in enumerate(items):
            self.assertTrue(Product.objects.filter(pk=item.product.pk).values_list('quantity')[0],
    ...
    def test_order_item_get_product_cost(self):
        ...
        self.assertEqual(self.order_item_2.get_item(self.order_item_1.pk), self.order_item_1)
