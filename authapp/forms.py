...
class BaseRegisterForm(UserCreationForm):
    ...
    avatar = forms.ImageField(
        widget=forms.FileInput(
            attrs={"class": "form-control", "placeholder": "Загрузите аватар"}
        ),
        required=False,
    )
...
class ShopUserRegisterForm(BaseRegisterForm):
    age = forms.IntegerField(
        widget=forms.NumberInput(
            attrs={"class": "form-control", "placeholder": "Введите Ваш возраст"}
        ),
        required=True,
    )
    ...
    def save(self):
        ...
        salt = hashlib.sha1(str(random.random()).encode(encoding="utf8")).hexdigest()[
            :6
        ]
        user.activation_key = hashlib.sha1(
            (user.email + salt).encode(encoding="utf8")
        ).hexdigest()
        user.save()
        return user
...
class ShopUserProfileExtraForm(forms.ModelForm):
    gender = forms.ChoiceField(
        choices=ShopUserProfile.GENDER_CHOICES,
