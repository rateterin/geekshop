...
class TestModelsMethods(TestCase):
    ...
    def test_user_is_activation_key_expired(self):
        ...
        self.assertFalse(
            self.user_activation_key_not_expired.is_activation_key_expired()
        )

    def test_user_profile_created(self):
        self.assertTrue(
            ShopUserProfile.objects.select_related()
            .filter(user=self.user_activation_key_expired)
            .exists()
        )
        self.assertTrue(
            ShopUserProfile.objects.select_related()
            .filter(user=self.user_activation_key_not_expired)
            .exists()
        )
